...
if __name__ == "__main__":
    ...
    dataset = FishEyeDataset("data", 'test')
    test_loader = DataLoader(dataset, 1, False, num_workers=0)
    ...
    with torch.no_grad():
        for idx, (img, ori_img, label) in enumerate(test_loader):
            # print(idx)
            img = img.cuda()
            # print(img[0][0][0])
            pred = model(img).cpu()
            ...
            pred = pred.tolist()
            label = (label.numpy().reshape(-1) / 10).tolist()

            img = img[0].permute(1, 2, 0).cpu().numpy()
            img = img * 128 + 128
            img = img.astype(np.uint8)
            undistorted_img = apply_distort(img, pred, 256, 256)
            ori_img = ori_img.numpy()[0]

            out_img = np.hstack([img, undistorted_img, ori_img])
            cv2.imwrite(str(idx)+".jpg", out_img)

            ssim_value.append(ssim(undistorted_img, ori_img))
            mse_value.append(mse.numpy() * 10)
            psnr_value.append(psnr(undistorted_img, ori_img))

            print(f"SSIM:{np.mean(ssim_value)}")
            print(f"MSE:{np.mean(mse_value)}")
            print(f"PSNR:{np.mean(psnr_value)}")
            # print(f"saved img:{idx}")
